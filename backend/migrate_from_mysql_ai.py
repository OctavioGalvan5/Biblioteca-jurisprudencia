...
Versión con Extracción de Metadatos y Jueces por IA desde PDF.

Por cada sentencia con drive_link:
  1. Verifica que no sea duplicado (SHA-256 del PDF)
  2. Descarga el PDF desde Google Drive
  3. Sube a MinIO
  4. Extrae texto completo (con OCR fallback si es escaneado)
  5. Extrae metadatos con IA (GPT-4o-mini) desde el texto extraído
  6. Extrae jueces firmantes usando Claude Vision de la última página (fallback a MySQL)
  7. Genera chunks + embeddings
  8. Vincula jueces con fuzzy matching — crea los que no existen

Uso (desde backend/, con el venv activo):
    python migrate_from_mysql_ai.py              # migra todo
    python migrate_from_mysql_ai.py --limit 10   # solo 10 para probar
    python migrate_from_mysql_ai.py --offset 50  # arranca desde la fila 50
    python migrate_from_mysql_ai.py --inspect    # muestra 5 filas de ejemplo
"""
...
from app.models.database_models import (
    Sentencia, SentenciaChunk, Juez, SentenciaJuez, Instancia, Organo,
...
def extract_jueces_via_vision(pdf_bytes: bytes, db) -> list[str]:
    """Fallback: extract judge names from the PDF signature page using Claude Vision."""
...
def _process_row(row: dict, i: int, total: int) -> str:
    ...
    try:
        ...
        # 1. Extraer metadatos con IA (GPT-4o-mini)
        print(f"       -> Extrayendo metadatos con IA (GPT)...")
        try:
            extracted_metadata = ai_extractor.extract_metadata_from_text(contenido)
        except Exception as e:
            print(f"       -> Error en extracción con IA: {e}")
            extracted_metadata = {}

        # Parsear fecha extraída por IA
        fecha_sentencia = None
        if extracted_metadata.get("fecha_sentencia"):
            try:
                fecha_sentencia = datetime.strptime(extracted_metadata["fecha_sentencia"], "%Y-%m-%d").date()
            except Exception:
                pass

        # Usar campos extraídos con IA prioritariamente, con fallback a MySQL
        caratula = extracted_metadata.get("caratula") or row.get("caratula") or f"Sin carátula (MySQL ID {row['id']})"
        nro_expediente = extracted_metadata.get("nro_expediente") or row.get("numero_expediente")
        
        # Resolver relaciones para instancia y órgano con get_or_create
        instancia_raw = extracted_metadata.get("instancia") or row.get("instancia")
        organo_raw = extracted_metadata.get("organo") or row.get("juzgado")
        
        instancia_obj = get_or_create_instancia(db, instancia_raw)
        organo_obj = get_or_create_organo(db, organo_raw)
        
        jurisdiccion = extracted_metadata.get("jurisdiccion") or map_jurisdiccion(row.get("jurisdiccion"))
        ...
        nueva = Sentencia(
            ...
            fecha_sentencia=fecha_sentencia or row.get("fecha_sentencia"),
        ...
        if not nombres_jueces:
            ...
            if not _jueces_raw_useless(jueces_raw):
                print(f"       -> Fallback: usando jueces de MySQL...")
        ...
        print(f"  [{i}/{total}] OK | {len(chunks)} chunks | {n_jueces} jueces | {caratula[:60]}")
        return "ok"
